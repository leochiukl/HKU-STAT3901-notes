...
\midrule
expression&\(\ax*{\angl{T_x}}\) or \(\displaystyle\int_{0}^{\infty}e^{-\delta t}\indicset{T_x>t}\dd{t}\)
&\(\displaystyle \frac{1-\Ax*{x}}{\delta}\) or \(\displaystyle\int_{0}^{\infty}e^{-\delta t}\px[t]{x}\dd{t}\)
...
t}\indicset{T_x>t}\dd{t}\), it can be understood intuitively as ``sum'' of
pure endowment with ``infinitesimal'' survival benefit
\faIcon{hand-holding-usd} ``\(\dd{t}\)''. Based on this
understanding, the APV formula \(\displaystyle\int_{0}^{\infty}e^{-\delta
t}\indicset{T_x>t}\dd{t}\) is ``summing up'' the expected present value of
\faIcon{hand-holding-usd} every \(t\)-year ``infinitesimal'' pure endowment:
\[
\underbrace{\dd{t}e^{-\delta t}}_{\text{PV of payment}}\times\underbrace{\px[t]{x}}_{\text{prob.\ of surviving at time \(t\)}}.
\]
\end{intuition}

\begin{note}
Note that \(\displaystyle \int_{0}^{\infty}e^{-\delta t}\indicset{T_x>t}\dd{t}
=\int_{0}^{T_x}e^{-\delta t}\dd{t}
...
\[
\expv{\int_{0}^{\infty}e^{-\delta t}\indicset{T_x>t}\dd{t}}
=\int_{0}^{\infty}\expv{e^{-\delta t}\indicset{T_x>t}}\dd{t}
=\int_{0}^{\infty}e^{-\delta t}\px[t]{x}\dd{t}
\]
...
&p.v.r.v.&APV&variance\\
\midrule
expression&\(\ax*{\angl{T_x\wedge n}}\) or \(\displaystyle\int_{0}^{n}e^{-\delta t}\indicset{T_x>t}\dd{t}\)
&\makecell{\(\displaystyle \frac{1-\Ax*{\endowxn}}{\delta}\) or 
\(\displaystyle \ax*{x}-\Ex[n]{x}\ax*{x+n}\)\\
or \(\displaystyle\int_{0}^{n}e^{-\delta t}\px[t]{x}\dd{t}\)}
...
&p.v.r.v.&APV \\
\midrule
expression&\(\ax*[u|]{\angl{T_x-u}}\indicset{T_x>u}\) or \(\displaystyle\int_{u}^{\infty}e^{-\delta t}\indicset{T_x>t}\dd{t}\)
&\makecell{\(\ax*{x}-\ax*{x:\angl{u}}\) or 
\(\Ex[u]{x}\ax*{x+u}\)\\
or \(\displaystyle\int_{u}^{\infty}e^{-\delta t}\px[t]{x}\dd{t}\)} \\
notation&\(Y\)&\(\ax*[u|]{x}\)\\
\bottomrule
\end{tabular}
...
&\(\ax*{\angl{n}}+\ax*[n|]{x}\)\\
notation&\(Y\)&\(\ax*{\overline{x:\angl{n}}}\)\\
\bottomrule
\end{tabular}
...
of) period we require the ``present value'' (at the beginning of period) of the
``discrete'' payment \faIcon{hand-holding-usd} to be the same as the whole
payment stream (in the framework) in that period:
...
\fill[] (4,0) circle [radius=0.05]
node[below] {2}
node[above, gray!20!white] {\faIcon{hand-holding-usd}};
\fill[] (6,0) circle [radius=0.05]
node[below] {3}
node[above] {\faIcon{hand-holding-usd}};
\fill[] (8,0) circle [radius=0.05]
...
node[midway, above, yellow!70!black] {stream};
\draw[-Latex, ForestGreen] (1.2,0.7) to[bend right] (0,0.3);
\draw[-Latex, ForestGreen] (2,0.2) to[bend left] (0,0.2);
\draw[magenta, very thick, decorate,decoration={mirror, brace, amplitude=5pt}] (0,0.4) -- (0,0.1)
node[midway, left=0.2cm, font=\small] {\faIcon{equals}};
\draw[-Latex, orange] (5.2,0.6) to[bend right] (4,0.3);
\draw[-Latex, orange] (6,0.2) to[bend left] (4,0.2);
\draw[magenta, very thick, decorate,decoration={mirror, brace, amplitude=5pt}] (4,0.4) -- (4,0.1)
node[midway, left=0.2cm, font=\small] {\faIcon{equals}};
\end{tikzpicture}
...
would involve ``\(\dd{t}\)'' --- the benefit paid in an ``infinitesimal'' time interval
...
t}}{\color{magenta}\px[t]{x}}{\color{brown}\dd{t}}
\]
(\({\color{brown}t\dd{t}}\): ``benefit payment''; \({\color{violet}e^{-\delta
t}}\): ``discount factor''; \({\color{magenta}\px[t]{x}}\): ``prob.\ of
triggering payment'', for every ``infinitesimal'' time interval \([t,t+\dd{t}]\)).
...
\draw[-Latex, ForestGreen] (1.7,0.2) to[bend right] (0,0.1);
\draw[-Latex, ForestGreen] (6.2,1.4) ..controls (3,3) and (2,3).. (0,0.3);
\node[brown] () at (1,2.2) {\(\times v\px{x}\)};
\end{tikzpicture}
