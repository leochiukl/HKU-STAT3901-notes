\subsection*{\Cref{sect:surv-dist}}
...
\item \labelcref{it:de-moivre-dist-surv}: distribution and survival function for De Moivre's Law
\item \labelcref{it:gen-de-moivre-surv}: survival function for De Moivre's Law
...
\item \labelcref{it:udd-only-length}: property that ``\(\qx{}\)'' probability only
depends on the length of time interval covered under UDD
\item \labelcref{it:udd-fom-fmla}: formula for \(\mu_{x+t}\) in terms of
\(\qx{x}\) under UDD
