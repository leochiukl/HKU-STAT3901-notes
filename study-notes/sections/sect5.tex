...
\begin{enumerate}
\item \defn{Present value of future loss at issue}, from the insurer's \faIcon{building}
...
\item The gross (net) loss at issue is denoted by \defn{\(L_0^g\)}
(\defn{\(L_0^n\)} resp.).

\begin{remark}
\item If there is no risk of ambiguity or one would like to refer to either of gross
and net losses at issue at the same time, one may drop the superscript and just
write \defn{\(L_0\)}.
...
\begin{enumerate}
\item \defn{initial expenses}: expenses incurred initially (at time of policy
issue, i.e., time 0);
\item \defn{renewal expenses}: expenses incurred each time a premium
...
0);
\item \defn{termination expenses} (or \defn{settlement expenses}): expenses
incurred when a policy terminates \faIcon{hourglass-end} (e.g., death
...
\labelcref{it:prem-ben-conventions};
...
\defn{\(\Px{x}\)};
\item term life insurance \faIcon{clock}\textbar\faIcon{shield-alt}: \defn{\(\Px{\:\termxn}\)};
\item endowment insurance \faIcon{clock}\textbar\faIcon{shield-alt}\textbar\faIcon{donate}: \defn{\(\Px{\endowxn}\)};
\item pure endowment \faIcon{donate}: \defn{\(\Px{\pureendowxn}\)}.
\end{enumerate}
