...
\item[(S3)] \(S_x(t)\) is a nonincreasing/decreasing function of \(t\).
\end{enumerate}
\end{proposition}
...
\[
dA(t)=A(t)\delta_t\dd{t},\quad\text{or}\quad
\delta_t=\frac{A'(t)}{A(t)},
\]
where \(A(t)\delta_t\dd{t}\) may be understood intuitively as ``interest earned in
\([t,t+\dd{t}]\)''.
...
\]
\begin{intuition}
We can express this as \(\dd{S_0(t)}=-S_0(t)\mu_t\dd{t}\), and \(S_0(t)\mu_t\dd{t}\) may be
intuitively understood as ``drop in survival probability in \([t,t+\dd{t}]\)'' or
``probability of death in \([t,t+\dd{t}]\)''.
...
\[
\dv{}{t}S_x(t)
=\dv{}{t}\frac{S_0(x+t)}{S_0(x)}
=\frac{1}{S_0(x)}\dv{}{t}S_0(\underbrace{x+t}_{u})
=\frac{S_0'(u)}{S_0(x)}\underbrace{\dv{u}{t}}_{1},
\]
and
...
\begin{pf}
Note that \(\displaystyle\mu_{x}(s)=\dv{}{s}\ln S_x(s)\). Then, taking
...
\begin{intuition}
Similar to the intuition in \labelcref{it:fom}, \(\px[t]{x}\mu_{x+t}\dd{t}\) may be
interpreted ``probability of death in time interval \([t,t+\dd{t}]\)''. On the
other hand, \(f_x(t)\dd{t}\) may also be understood as ``probability of death in
time interval \([t,t+\dd{t}]\)''.
\end{intuition}
\end{enumerate}
\end{proposition}
...
\[
\eringx{x}=\int_{0}^{\infty}t\,\px[t]{x}\mu_{x+t}\dd{t}.
\]
...
\[
\eringx{x}=\int_0^{\infty}\px[t]{x}\dd{t}.
\]
\end{proposition}
...
\[
T_x=\int_{0}^{T_x}1\dd{t}
=\int_{0}^{\infty}\indicset{t<T_x}\dd{t}.
\]
\begin{note}
...
\[
\expv{T_x}=\expv{\int_{0}^{\infty}\indicset{t<T_x}\dd{t}}
=\int_{0}^{\infty}\expv{\indicset{T_x>t}}\dd{t}
=\int_{0}^{\infty}\prob{T_x>t}\dd{t},
\]
...
\[
\expv{T_x^2}=2\int_{0}^{\infty}t\,\px[t]{x}\dd{t}.
\]
\end{proposition}
\begin{pf}
By definition, we have:
\[
\expv{T_x^2}=\int_{0}^{\infty}t^2\px[t]{x}\mu_{x+t}\dd{t}
=-\int_{0}^{\infty}t^2\dv{}{t}\px[t]{x}\dd{t}.
\]
Integrating it by parts gives
\[
\underbrace{-[t^2\px[t]{x}]_0^{\infty}}_{0}-\int_{0}^{\infty}2t\px[t]{x}\dd{t}
\]
...
\[
\eringx{\endowxn}=\int_{0}^{n}\px[t]{x}\dd{t}.
\]
\end{proposition}
\begin{pf}
...
=\expv{\int_{0}^{\infty}\indicset{t<T_x}\indicset{t<n}\dd{t}}
=\expv{\int_{0}^{n}\indicset{t<T_x}\dd{t}}.
\]
\end{pf}
\end{enumerate}
...
\begin{pf}
``\(\Leftarrow\)'' follows from the definition of floor function
\(\lfloor\cdot\rfloor\). ``\(\Rightarrow\)'' can be proved by contrapositive
easily.
\end{pf}
...
\[
\expv{K_x}=\expv{\sum_{k=1}^{K_x}1}
=\expv{\sum_{k=1}^{\infty}\indicset{k\le K_x}}
=\sum_{k=1}^{\infty}\prob{K_x\ge k}
=\sum_{k=1}^{\infty}\prob{T_x\ge k}
=\sum_{k=1}^{\infty}\px[k]{x}.
\]
\begin{note}
When \(K_x=0\), we define \(\sum_{k=1}^{K_x}1=0\).
\end{note}
\end{pf}
...
+\underbrace{\int_{n}^{\infty}\px[t]{x}\dd{t}}_{\px[n]{x}\int_{0}^{\infty}\px[t]{x+n}\dd{t}}
...
\node[] () at (1,0.5) {\faIcon{user}};
\node[] () at (1,-0.5) {\faIcon{skull}};
\node[] () at (1.5,-0.3) {\faIcon{user}};
\node[] () at (-1,0) {\faIcon{user}};
\node[] () at (-1.5,-0.5) {\faIcon{user}};
\node[] () at (0.2,0.5) {\faIcon{user}};
\end{tikzpicture}
...
\node[] () at (1,-0.5) {\faIcon{skull}};
\node[] () at (1.5,-0.3) {\faIcon{user}};
\node[] (l2) at (-1,0) {\faIcon{user}};
\node[] (l1) at (-1.5,-0.5) {\faIcon{user}};
\node[] () at (0.2,0.5) {\faIcon{user}};
...
table, e.g.:
\begin{enumerate}
\item \(\displaystyle \px[t]{x}=\frac{\lx{x+t}}{\lx{x}}\)
\item \(\displaystyle \qx[t]{x}=1-\frac{\lx{x+t}}{\lx{x}}\)
\item \(\displaystyle \qx[u|t]{x}=\frac{\lx{x+u}-\lx{x+u+t}}{\lx{x}}\)
\end{enumerate}
...
=\frac{\dv{}{t}\qx[t]{x}}{1-\qx[t]{x}}
=\frac{\dv{}{t}tq_x}{1-tq_x}
=\frac{q_x}{1-tq_x}.
\]
\end{pf}
\end{enumerate}
...
\[\mu_x^*=\mu_{x+t}
=-\frac{1}{\px[t]{x}}\dv{}{t}\px[t]{x}
=-\frac{1}{(\px{x})^t}\dv{}{t}(\px{x})^t
...
incorporated).
\item As a result, previous formulas ``modelled''/``started'' at age \(x\)
