...
time \(t\), denoted by \defn{\(\mu_t\)} (or \defn{\(\mu_0(t)\)}), is defined as
\[
...
\]
\begin{note}
\defn{\(\indicset{\cdot}\)} denotes the indicator function.
\end{note}
...
``capped'' to \(n\) years: \(T_x\,\text{\defn{\(\wedge\)}}\, n\) (i.e.,
...
(Actuarial notation: \defn{\(\eringx{\endowxn}\)})
